...
class Analizador:
        ...
        for i in range(len(self.contenidoArchivo)):
            ...
            if self.estadoActual == self.inicio:
                ...
                if not self.trans.isdigit() and self.trans.isalnum(): #Es letra
                    ...
                    if self.trans == '#':
                        ...
                        self.tokens += 1
                        self.tokensEncontrados += [[self.trans, "Inicio agrupación", ""]]
                        print("Se encontró un inicio de agrupación: %s" % self.trans)
                        self.estadoActual = 'PP'
                    elif self.trans == '{':
                        self.tokens += 1
                        self.tokensEncontrados += [[self.trans, "", ""]]
            ...
            #identificación de string con comillas simples
                ...
                if self.trans == '<':
                    ...
                    self.tokens += 1
                    print("Se encontró el operador para incluir librería: %s" % self.contenidoArchivo[i - 1])
                    self.estadoActual = self.inicio
                else:
                    self.tokens += 1
                    print("Se encontró el símbolo de comparación 'menor que': %s" % self.contenidoArchivo[i - 1])
                    self.estadoActual = self.inicio
    ...
    def nombreID(self, inicio, fin):
        ...
        # self.var = "".join(arregloID)
        # self.vvvv.append(self.var)
